...
export const Gestures = ['tap', 'doubletap', 'pan', 'pinch', 'press', 'swipe', 'rotate'] as const
...
class Touch<T extends HTMLElement = HTMLElement> {
	...
	constructor(el: T, options?: InitOptionsType) {
		...
		this._hammerManager.add([
			new Hammer.Rotate({ event: 'rotate', pointers: 2, threshold: 0 }),
			new Hammer.Pinch({ event: 'pinch', pointers: 2, threshold: 0 }),
			new Hammer.Swipe({ event: 'swipe', direction: Hammer.DIRECTION_ALL }),
			new Hammer.Pan({ event: 'pan', direction: Hammer.DIRECTION_ALL }),
			new Hammer.Tap({ event: 'doubletap', taps: 2, interval: 500 }),
		...
		for (const gesture of Object.keys(multiGestureOptions)) {
			...
			this.reWriteGestureConfig(gesture as GestureType, multiGestureOptions[gesture])
		}
	}
	...
	public addEventListener(eventName: GestureEventType, callback: GestureEventCallbackType, options?: GestureEventOptionsType) {
		if (!this._hammerManager || !this._eventCallbackMap) {
			console.error('touch instance was destroyed.')
			return
		}
