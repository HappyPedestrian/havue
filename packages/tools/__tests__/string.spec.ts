...
describe('tools: string', () => {
  ...
  test('subStrByByteLen', () => {
    ...
    expect(subStrByByteLen('!啊bcd额', 3)).toBe('!')
    expect(subStrByByteLen('!啊bcd额', 4)).toBe('!啊')
    expect(subStrByByteLen('!啊bcd额', 5)).toBe('!啊b')
    expect(subStrByByteLen('!啊bcd额', 6)).toBe('!啊bc')
    expect(subStrByByteLen('!啊bcd额', 7)).toBe('!啊bcd')
    expect(subStrByByteLen('!啊bcd额', 8)).toBe('!啊bcd')
    expect(subStrByByteLen('!啊bcd额', 9)).toBe('!啊bcd')
    expect(subStrByByteLen('!啊bcd额', 10)).toBe('!啊bcd额')
  })
})
