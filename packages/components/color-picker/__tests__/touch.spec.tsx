...
describe('ColorPicker touch', () => {
  describe.sequential('drag', () => {
    ...
    it.sequential('left top', async () => {
      area.trigger('touchstart', {
        touches: [new Touch({ clientX: 0, clientY: 0, identifier: 1, target: area.element })]
      })
    ...
    it.sequential('left bottom', async () => {
      area.trigger('touchstart', {
        touches: [new Touch({ clientX: 0, clientY: CANVAS_HEIGHT, identifier: 1, target: area.element })]
      })

      await sleep(50)
      const touchmove = new TouchEvent('mousemove', {
        touches: [new Touch({ clientX: 0, clientY: CANVAS_HEIGHT, identifier: 1, target: area.element })]
      })
      document.body.dispatchEvent(touchmove)
      await sleep(50)
    ...
    it.sequential('right top', async () => {
      ...
      area.trigger('touchstart', {
        touches: [new Touch({ clientX: targetPoint.x, clientY: targetPoint.y, identifier: 1, target: area.element })]
      })
    ...
    it.sequential('darken', async () => {
      ...
      slider.trigger('touchstart', {
        touches: [new Touch({ clientX: centerPoint.x, clientY: centerPoint.y, identifier: 1, target: area.element })]
      })
      await sleep(50)
      slider.trigger('touchmove', {
        touches: [new Touch({ clientX: centerPoint.x, clientY: centerPoint.y, identifier: 1, target: area.element })]
      })
      await sleep(50)
      ...
      await sleep(50)
      slider.trigger('touchmove', {
        touches: [new Touch({ clientX: leftPoint.x, clientY: leftPoint.y, identifier: 1, target: area.element })]
      })
      await sleep(50)
